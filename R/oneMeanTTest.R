...
#' @param alpha Numeric. Significance level (0 < alpha < 1). Default is 0.05.
...
oneMeanTTest <- function(x, mu, ha, data, alpha = 0.05, plot = TRUE) {
  ...
  if (plot) {
    ...
    m <- mean(col_data); s <- sd(col_data)
    x_vals <- seq(min(col_data), max(col_data), length = 200)
    y_vals <- dnorm(x_vals, mean = m, sd = s) * diff(hist_output$mids[1:2]) * length(col_data)
  ...
  z_input <- switch(ha, "greater" = qnorm(1 - alpha), "less" = qnorm(alpha), "not equal" = qnorm(1 - alpha/2))
  term_crit <- (z_input^2 / df) * ((8*df + 3) / (8*df + 1))^2
  t_critical <- sqrt(df * (exp(term_crit) - 1))
  if (ha == "less") t_critical <- -t_critical

  # Hastings p-value approximation (4-term, intentional)
  ...
  Phi_hastings <- function(z) {
    ...
    t_val <- 1 + a1*z + a2*z^2 + a3*z^3 + a4*z^4
    return(1 - 0.5 * t_val^(-4))
  }
  ...
  # Return S3 object
  result <- list(
    variable = x, n = n, sample_mean = xbar, sample_sd = sd_val,
    hypothesized_mean = mu, t_statistic = t_stat, df = df,
    critical_value = t_critical, p_value = p_value, confidence_interval = ci,
    alternative = ha, alpha = alpha, decision = decision, interpretation = interpretation,
    shapiro_stat = shapiro_test$statistic, shapiro_p = shapiro_test$p.value,
    is_normal = data_is_normal, outlier_count = length(outliers), outliers = outliers
  )
...
print.oneMeanTTest <- function(x, ...) {
  ...
  cat(paste0((1 - x$alpha)*100, "% CI:     "), "[", ci_lower_str, ", ", ci_upper_str, "]\n")
